...
local MAJOR, MINOR = "LibNameplates-1.0", 39
    ...
    f:SetScript("OnUpdate", function(this, elapsed)
        ...
        for frame in pairs(lib.nameplates) do
            ...
            if frame:IsShown() and lib:IsTarget(frame) then
                ...
                lib.callbacks:Fire(callbackOnTarget, lib.fakePlate[frame] or frame)
                --if not lib.plateGUIDs[frame] then -- XXX Disabled for now since target detection has (recently?) become unreliable
                --    FoundPlateGUID(frame, UnitGUID(unitID), unitID)
                --end
                break
            end
...
do
    function lib:IsTarget(frame)
        ...
        -- if tidy plate is here... it stores the original alpha in .alpha and set the actual one to 1
        if frame.extended and frame.alpha then
            return frame.alpha == 1 and UnitExists("target") or false
        end
    ...
    function lib:GetTargetNameplate()
        if UnitExists("target") then
            for frame in pairs(self.nameplates) do
                ...
                alpha = frame.extended and frame.alpha or frame:GetAlpha();

                if frame:IsShown() and alpha == 1 then
